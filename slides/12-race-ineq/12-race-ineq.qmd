...
```{r mobCZ, fig.cap="Source: [Chetty, Friedman, Hendren, Jones and Porter (2018)](https://utsa.instructure.com/courses/79928/files/folder/papers?preview=13306202)", echo=FALSE, out.width = '90%'}
...
- If the *only* wealth-accumulating condition you equalize is income (the target of most traditional anti-poverty policy), the wealth gap would **still** take 200+ years to close

- Two kinds of policies can attack the wealth gap:

    - **Stock-based** (e.g., reparations, baby bonds): directly change the wealth you start with. These can move the gap immediately
    - **Flow-based** (e.g., financial inclusion, portfolio diversification, education and labor market policies): change the savings and returns going forward

- The authors argue these are **complementary**: a one-time transfer without equalizing the flows would wear off over generations; equalizing the flows without addressing the stock would take centuries

- Closing the wealth gap requires **both**
